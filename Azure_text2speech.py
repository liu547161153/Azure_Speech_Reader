...
class Azure_text2speech:
    ...
    async def text_to_speech(self, text, style=None, voice_name="zh-CN-XiaoxiaoNeural", role=None, rate=1.0):
        ...
        # 处理不同的情况
        if style and role:
            body += f'<mstts:express-as role="{role}" style="{style}" styledegree="2">'
        elif style:
            body += f'<mstts:express-as style="{style}" styledegree="2">'
        elif role:
            body += f'<mstts:express-as role="{role}" styledegree="2">'
            # 使用 prosody 标签设置语速
